...
import { getAllNonEntityDomains } from '../../data/domains';
import type { NonEntity } from '../../types';
...
export default function NonEntityPage() {
  const { nonEntities: ctxNonEntities } = useData();
  ...
  const loadFirebaseCounts = async () => {
    ...
    try {
      const nonEntities = await nonEntityApi.getAll();
  ...
  useEffect(() => {
    loadFirebaseCounts();
    const storedHistory = JSON.parse(localStorage.getItem('l1ToL2UploadHistory_nonEntity') || '[]');
    if (Array.isArray(storedHistory)) setUploadHistory(storedHistory);
    const storedJobs = JSON.parse(localStorage.getItem('l1ToL2ScheduledJobs_nonEntity') || '[]');
    if (Array.isArray(storedJobs)) setScheduledJobs(storedJobs);
  }, []);

  useEffect(() => {
    localStorage.setItem('l1ToL2UploadHistory_nonEntity', JSON.stringify(uploadHistory));
  }, [uploadHistory]);

  useEffect(() => {
    localStorage.setItem('l1ToL2ScheduledJobs_nonEntity', JSON.stringify(scheduledJobs));
  }, [scheduledJobs]);

  const nonEntities = fbNonEntities.length > 0 ? fbNonEntities : ctxNonEntities;

  const generateJSONData = (domainCode: string) => {
    const data = nonEntities.filter(n => n.primary_domain === domainCode);
    return JSON.stringify(data.map(n => ({
      pk: n.non_entity_pk,
      name: n.non_entity_name,
      domain: n.primary_domain,
      category: n.category_pk,
      category_name: n.category_name,
      type: n.type_pk,
      visibility_type: n.visibility_type,
      status: n.status,
      created_at: n.createdAt,
      updated_at: n.updatedAt,
      branch: {
        geo: `${n.stateId}/${n.districtId}/${n.talukId}${n.cityVillageId ? `/${n.cityVillageId}` : ''}${n.areaId ? `/${n.areaId}` : ''}${n.streetId ? `/${n.streetId}` : ''}`,
        zone: n.zone_pk || null,
        dct: `${n.primary_domain}/${n.category_pk}${n.type_pk ? `/${n.type_pk}` : ''}`,
        site: n.website_zone_entity_id || null,
      },
      phone: n.phone || null,
      linkedEntities: n.linkedEntities,
    })), null, 2);
  };

  const handleDownloadJSON = (domainCode: string) => {
    const fileName = `${domainCode.toLowerCase()}.non-entity.json`;
  ...
  const confirmPushToL2 = async () => {
    ...
    setIsUploading(true);
    try {
      const fileName = `${pushModalDomain.toLowerCase()}.non-entity.json`;
      const jsonData = generateJSONData(pushModalDomain);
      const finalDriveLink = driveLinkInput.trim() || `https://drive.google.com/file/d/mock-${Date.now()}-${pushModalDomain}-non-entity/view`;
      const nonEntityCount = nonEntities.filter(n => n.primary_domain === pushModalDomain).length;

      const record: UploadRecord = {
        id: `UPLOAD-${Date.now()}`,
        domain: pushModalDomain,
        entityType: 'non-entity',
        fileName,
        driveLink: finalDriveLink,
        uploadedAt: new Date().toISOString(),
        uploadedBy: 'admin',
        recordCount: nonEntityCount,
        status: 'success',
      };
      ...
      alert(`Non-Entity data pushed to L2 successfully!\nFile: ${fileName}\nDrive Link: ${finalDriveLink}\nRecords: ${nonEntityCount}`);
    } catch (err) {
      const record: UploadRecord = {
        id: `UPLOAD-${Date.now()}`,
        domain: pushModalDomain,
        entityType: 'non-entity',
        fileName: `${pushModalDomain.toLowerCase()}.non-entity.json`,
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-surface-200 shadow-lg p-6">
        <h3 className="text-lg font-extrabold text-surface-900 font-display tracking-tight mb-5">Non-Entity Data Export</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 bg-white rounded-xl border border-surface-200 text-sm text-surface-900 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500"
            >
              <option value="">All Non-Entity Domains</option>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-surface-100">
                {nonEntityDomains
                  ...
                  .map(domain => {
                    const nonEntityCount = nonEntities.filter(n => n.primary_domain === domain.code).length;
                    return (
                      <tr key={domain.code} className="hover:bg-surface-50/80 transition-colors duration-150">
                        ...
                        <td className="px-5 py-4">
                          <span className="text-sm font-extrabold text-emerald-600">{nonEntityCount}</span>
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex justify-end gap-2 flex-wrap">
                            ...
                            <button onClick={() => handleDownloadJSON(domain.code)} className="p-2 text-surface-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all" title="Download JSON">
                              <Download className="w-4 h-4" />
                            </button>
                            <button onClick={() => pushToL2(domain.code)} disabled={isUploading} className="p-2 text-surface-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all disabled:opacity-40" title="Push to L2">
                    ...
                    <tr key={job.id} className="hover:bg-surface-50/80 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`px-3 py-1.5 text-xs rounded-full font-extrabold ${
                          job.status === 'scheduled' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                  ...
                  {uploadHistory.map(upload => (
                    <tr key={upload.id} className="hover:bg-surface-50/80 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        {upload.driveLink ? (
                          <div className="flex items-center gap-2">
                            <a href={upload.driveLink} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:text-emerald-800 font-semibold" title={upload.driveLink}>
      ...
      <Modal isOpen={showPushModal} onClose={() => setShowPushModal(false)} title="Push Non-Entity Data to L2" maxWidth="lg">
        <div className="space-y-4">
          <div className="bg-slate-50 rounded-lg p-4">
            <p className="text-sm text-slate-600">Domain: <span className="font-bold text-slate-900">{pushModalDomain}</span></p>
            <p className="text-sm text-slate-600">Type: <span className="font-bold text-slate-900">Non-Entity</span></p>
            <p className="text-sm text-slate-600">Records: <span className="font-bold text-slate-900">{nonEntities.filter(n => n.primary_domain === pushModalDomain).length}</span></p>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={confirmPushToL2} disabled={isUploading} className="px-4 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed">
